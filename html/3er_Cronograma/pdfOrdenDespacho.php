<?php
	...
	$fecha=date("d-m-Y");
	$sql=$db->query("SELECT c.cotizacion, c.total, c.fecha, e.nombre, e.rif, e.direccion, e.telefono, e.contacto, ct.cod_tlf, c.descuento, c.desc_porcentaje, d.despacho, e.telefono2, e.correo, d.banco, d.deposito FROM cotizacion c INNER JOIN empresa e ON c.id_empresa=e.id_empresa INNER JOIN cod_tlf ct ON e.id_cod_tlf=ct.id_cod_tlf INNER JOIN despacho d ON c.id_cotizacion=d.id_cotizacion WHERE c.id_cotizacion=$id_cotizacion LIMIT 1;");
	if($db->rows($sql)>0){
		...
		$sql1=$db->query("SELECT o.cantidad, p.producto, m.modelo, a.nombre FROM ordenes o INNER JOIN productos p ON o.id_producto=p.id_producto INNER JOIN modelo m ON o.id_modelo=m.id_modelo INNER JOIN almacenadoras a ON o.id_almacenadora=a.id_almacenadora WHERE id_cotizacion=$id_cotizacion;");
		if($db->rows($sql1)>0){
			$contador=1;
