<?php
	...
	$sql=$db->query("SELECT c.cotizacion, c.fecha, c.total, c.total_num, e.nombre, c.unidades_total FROM cotizacion_juguetes c INNER JOIN empresa e ON c.id_empresa=e.id_empresa WHERE c.fecha BETWEEN '$desde' AND '$hasta' AND despacho=1 ORDER BY c.fecha;");
	if($db->rows($sql)>0){
		$encabezado="
