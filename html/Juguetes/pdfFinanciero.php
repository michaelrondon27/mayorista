<?php
	...
	$sql=$db->query("SELECT c.cotizacion, c.fecha, c.total, c.total_num, d.deposito FROM cotizacion_juguetes c INNER JOIN despacho_juguetes d ON c.id_cotizacion=d.id_cotizacion WHERE c.despacho=1 AND c.fecha BETWEEN '$desde' AND '$hasta' AND despacho=1 ORDER BY c.fecha;");
	if($db->rows($sql)>0){
		$encabezado="
