<?php
	...
	$fecha=date("d-m-Y");
	$sql=$db->query("SELECT c.cotizacion, c.total, c.fecha, e.nombre, e.rif, e.direccion, e.telefono, e.contacto, ct.cod_tlf, c.descuento, c.desc_porcentaje, d.despacho, e.telefono2, e.correo, d.banco, d.deposito FROM cotizacion_4to c INNER JOIN empresa e ON c.id_empresa=e.id_empresa INNER JOIN cod_tlf ct ON e.id_cod_tlf=ct.id_cod_tlf INNER JOIN despacho_4to d ON c.id_cotizacion=d.id_cotizacion WHERE c.id_cotizacion=$id_cotizacion LIMIT 1;");
	if($db->rows($sql)>0){
		...
		$sql1=$db->query("SELECT o.cantidad, p.producto, m.modelo, a.nombre FROM ordenes_4to o INNER JOIN productos_4to p ON o.id_producto=p.id_producto INNER JOIN modelo_4to m ON o.id_modelo=m.id_modelo INNER JOIN almacenadoras_4to a ON o.id_almacenadora=a.id_almacenadora WHERE id_cotizacion=$id_cotizacion;");
		if($db->rows($sql1)>0){
			$contador=1;
				...
				if($contador<=29){
					$productos="
						<tr>
							...
							<td class='abajo izquierda' align=center>".$data1[3]."</td>
							<td class='abajo izquierda' align=center></td>
							<td class='abajo' align=right></td>
						</tr>";
				}else if($contador>29 && $contador<=30){
					$productos="
						</table>
						<table>
							<tr>
		...
		$footer="
			...
			<table>
				<tr><td colspan='2' class='abajo' style='font-size: 9px;'><strong>NOTA: Válido por 10 días hábiles a partir de la fecha de emisión Únicamente con la nota de entrega original se podrá realizar el retiro de los productos y copia de cédula de identidad. </strong></td>
				</tr>
				<tr><td colspan='2' class='abajo' style='font-size: 9px;'><strong>Importante: Para la confirmación de está orden las Almacenadoras pueden contactar a COMERSSO, S.A. por los siguiente números telefónicos: 0212-509-67-57/6838/6789 </strong></td>
				</tr>
				<tr>
				...
				<tr><td colspan='2' class='abajo'>Almacenadora Transgar: Av. Henry Ford. Parcela B4. Urb. Ind. Las Caracaritas. Zona Ind. Sur II; Valencia, Edo. Carabobo.</td>
				</tr>
