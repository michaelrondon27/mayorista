<?php
	...
	$tlf=$_cod_tlf[$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['id_cod_tlf']]['cod_tlf']."-".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['telefono'];
	if($_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['id_cod_tlf2']>0){
		$otrotlf=$_cod_tlf[$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['id_cod_tlf2']]['cod_tlf']."-".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['telefono2'];
	}else{
		$otrotlf="";
	}
	$fecha=date("Y", strtotime($_cotizacion4to[$id_cotizacion]['fecha']));
	if($fecha>=2017){
	...
	$encabezado="
		<div class='contenedor'>
			...
			<div>
				Cotizaci&oacute;n: ".$_cotizacion4to[$id_cotizacion]['cotizacion']."
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				Fecha: ".$fec=date("d-m-Y", strtotime($_cotizacion4to[$id_cotizacion]['fecha']))."
			</div>
			<div class='clear'></div>
			<table class='header'>
				<tr>
					<td colspan='2' class='abajo'>BENEFICIARIO: ".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['nombre']."</td>
				</tr>
				<tr>
					<td class='abajo izquierda'>RIF: ".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['rif']."</td>
					<td class='abajo '>DIRECCION: ".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['direccion']."</td>
				</tr>
				<tr>
					<td class='izquierda abajo'>TELÉFONO: ".$tlf."</td>
					<td class='abajo'>PERSONA DE CONTACTO: ".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['contacto']."</td>
				</tr>
				<tr>
					<td class='izquierda'>OTRO TELÉFONO: ".$otrotlf."</td>
					<td>CORREO: ".$_beneficiario[$_cotizacion4to[$id_cotizacion]['id_empresa']]['correo']."</td>
				</tr>
			</table>
			<table>
				<tr>
		...
		$sql=$db->query("SELECT o.cantidad, p.producto, m.modelo, m.precio, o.precio_total, a.nombre FROM ordenes_4to o INNER JOIN productos_4to p ON o.id_producto=p.id_producto INNER JOIN modelo_4to m ON o.id_modelo=m.id_modelo INNER JOIN almacenadoras_4to a ON o.id_almacenadora=a.id_almacenadora WHERE id_cotizacion=$id_cotizacion;");
		if($db->rows($sql)>0){
			$contador=1;
		...
		if($_cotizacion4to[$id_cotizacion]['desc_porcentaje']!=""){
			$descuento="
				<tr>
					<td colspan='3' align='right' class='abajo izquierda'>
						<strong>CANTIDAD TOTAL:</strong>
					</td>
					<td class='abajo izquierda' align=center>".$_cotizacion4to[$id_cotizacion]['unidades_total']."</td>
					<td align='right' class='abajo izquierda'>
						<strong>SUBTOTAL:</strong>
					</td>
					<td class='abajo izquierda' align=center>".$_cotizacion4to[$id_cotizacion]['subtotal']."</td>
				</tr>
				<tr>
					<td colspan='4' align='right' class='abajo izquierda'>
						<strong>DESCUENTO:</strong>
					</td>
					<td class='abajo izquierda' align=center>".$_cotizacion4to[$id_cotizacion]['desc_porcentaje']."%</td>
					<td class='abajo' align=center>".$$cotizacion4to[$id_cotizacion]['descuento']."</td>
				</tr>";
		...
		$footer="
				<tr>
					...
					<td align=right>".$_cotizacion4to[$id_cotizacion]['total']."</td>
				</tr>
			</table>
